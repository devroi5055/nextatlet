...
import { Home, LayoutTemplate, PanelLeft, Folder, Users, User2 } from 'lucide-react';
...
const Logo = () => {
  ...
  return (
    <Link className="flex items-center text-white" href={paths.home.getHref()}>
      <img className="h-8 w-auto" src="/logo.svg" alt={t('logoAlt')} />
...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  const navigation = [
    { name: t('nav.dashboard'), to: paths.app.root.getHref(), icon: Home },
    {
      name: t('nav.editor'),
      to: paths.app.editor.getHref(),
      icon: LayoutTemplate,
    },
    {
...
function Fallback({ error }: FallbackProps) {
  ...
  const message = error instanceof Error ? error.message : te('somethingWrong');
